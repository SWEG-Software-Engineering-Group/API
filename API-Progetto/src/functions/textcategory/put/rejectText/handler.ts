...
const putRejectText: ValidatedEventAPIGatewayProxyEvent<typeof schema> = async (event) => {
  try {
    ...
    await updateText(event.pathParameters.TenantId, event.pathParameters.Language, event.pathParameters.Category, event.pathParameters.Title.replaceAll("%20", " "), state.rifiutato);
